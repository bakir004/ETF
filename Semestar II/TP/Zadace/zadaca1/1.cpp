...
int BrojKvadrata(int n) {
  ...
  for (int i = sqrt(n); i >= sqrt(nKopija / 4); i--) {
    n = nKopija -
        i * i; // probavamo sve potpune kvadrate manje od originalnog 'n'
    ...
    for (int j = i; j >= 1; j--) {
      ...
      if (n - j * j >= 0) {
        ...
        if (trenutniBrojKvadrata <= 4 &&
            n == 0) { // ne zelimo ici dalje putem koji ima vise od 4 kvadrata
          minBrojKvadrata = std::min(minBrojKvadrata, trenutniBrojKvadrata);
          break;
        } // ako nismo uspjeli, oduzmi opet jos jednom kvadrat is prethodnog
          // koraka
        else if (n - j * j >= 0)
          j++;
      }
...
RazvrstajPoBrojuKvadrata(std::vector<int> niz, TretmanNegativnih tretman) {
...
int main() {
  ...
  try {
    std::array<std::vector<int>, 5> razvrstani =
        RazvrstajPoBrojuKvadrata(niz, TretmanNegativnih::PrijaviGresku);
