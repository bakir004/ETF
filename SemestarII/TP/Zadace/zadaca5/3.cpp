...
public:
  Sanduk(double tezina, std::string naziv, std::vector<int> tezine)
      : Spremnik(tezina, naziv), tezinePredmeta(tezine) {}
...
public:
  Vreca(double tezina, std::string naziv, double tezinaMaterije)
      : Spremnik(tezina, naziv), tezinaMaterije(tezinaMaterije) {}
  double DajUkupnuTezinu() const override;
  void Ispisi() const override {
...
double Vreca::DajUkupnuTezinu() const { return DajTezinu() + tezinaMaterije; }
...
public:
  Bure(double tezina, std::string naziv, double tezinaTecnosti,
       double zapremina)
      : Spremnik(tezina, naziv), tezinaTecnosti(tezinaTecnosti),
        zapremina(zapremina) {}
  double DajUkupnuTezinu() const override;
  void Ispisi() const override {
...
public:
  ...
  Spremnik *DodajBure(double tezina, std::string naziv, double tezinaTecnosti,
                      double zapremina);
  ...
  while (!ulazniTok.eof()) {
    ...
    try {
      ...
      case 'S': {
        ...
        noviSpremnici.push_back(
            std::make_shared<Sanduk>(tezina, naziv, tezine));
        break;
      }
      case 'V': {
        int tezinaMaterije;
        ...
        if (!ulazniTok)
          throw std::logic_error("Datoteka sadrzi besmislene podatke");
        noviSpremnici.push_back(
            std::make_shared<Vreca>(tezina, naziv, tezinaMaterije));
        break;
      }
      case 'B': {
        ...
        if (!ulazniTok)
          throw std::logic_error("Datoteka sadrzi besmislene podatke");
        noviSpremnici.push_back(
            std::make_shared<Bure>(tezina, naziv, specificnaTezina, zapremina));
        break;
      }
      }
      if (ulazniTok.bad())
...
Spremnik &Skladiste::DajNajlaksi() {
  ...
  return **std::min_element(spremnici.begin(), spremnici.end(),
                            [](const std::shared_ptr<Spremnik> &s1,
                               const std::shared_ptr<Spremnik> &s2) {
...
Spremnik *Skladiste::DodajSpremnik(Spremnik *p, bool predajeVlasnistvo) {
  ...
  if (predajeVlasnistvo) {
    ...
    std::shared_ptr<Spremnik> noviPointer =
        std::shared_ptr<Spremnik>(p->DajKopiju());
