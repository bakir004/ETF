...
Razlomak::Razlomak(std::initializer_list<Long> list) {
  if (list.size() == 1)
    ...
    Postavi(*list.begin(), *(list.begin() + 1));
  else
    throw std::domain_error("Neispravni parametri (inicijalizacijska lista)");
}
Razlomak::Razlomak(Long brojnik, Long nazivnik) { Postavi(brojnik, nazivnik); }

bool operator<(const Razlomak &r1, const Razlomak &r2) {
  return (long double)r1.brojnik / r1.nazivnik <
         (long double)r2.brojnik / r2.nazivnik;
}
bool operator>(const Razlomak &r1, const Razlomak &r2) {
  return (long double)r1.brojnik / r1.nazivnik >
         (long double)r2.brojnik / r2.nazivnik;
...
void Razlomak::TestirajPrekoracenjeSume(Long a, Long b) {
  if (LONG_LONG_MAX - std::abs(a) < std::abs(b))
...
Razlomak operator*(const Razlomak &r1, const Razlomak &r2) {
  ...
  return {(r1.brojnik / s) * (r2.brojnik / t),
          (r1.nazivnik / t) * (r2.nazivnik / s)};
}
Razlomak operator/(const Razlomak &r1, const Razlomak &r2) {
  ...
  return {(r1.brojnik / u) * (r2.nazivnik / r),
          (r1.nazivnik / r) * (r2.brojnik / u)};
}
Razlomak operator+(const Razlomak &r1, const Razlomak &r2) {
  Long r = Razlomak::NZD(r1.nazivnik, r2.nazivnik);
