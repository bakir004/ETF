...
fs.readFile("input.txt", "utf8", (err, data) => {
  ...
  tests.forEach((test) => {
    ...
    if (test.tools[2].execute && test.tools[2].execute.environment) {
      output +=
        test.name +
        (test.options ? ": Skriven" : ": Nije skriven") +
        " - Koristi vasu main() funkciju i stdin ulaz\n\nUlaz:\n\n";
      let stdin = test.tools[2].execute.environment.stdin;
      output += stdin;
    } else {
      output +=
        test.name + (test.options ? ": Skriven" : ": Nije skriven") + "\n";
      output +=
      ...
      codeSnippets.forEach((snippet) => {
        if (snippet.position === "main") output += "int main() {\n";

        output += snippet.code + "\n\n";
        if (snippet.position === "main") output += "    return 0;\n}\n\n";
      });
      output +=
        "========================================================================\n\n";
    }
    let expectedOutputArray = test.tools[2].execute.expect;
    if (expectedOutputArray.length > 1)
      output += "\nOvaj autotest ima vise validnih izlaza!\n";
    expectedOutputArray.forEach(
      (expectedOutput, i) =>
        (output +=
          "\nIzlaz" +
          (expectedOutputArray.length > 1 ? " " + (i + 1) : "") +
          ":\n\n" +
          expectedOutput +
          "\n\n")
    );
